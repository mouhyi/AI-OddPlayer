...
/**
 * Odd Game board representation. The board is represented as a two-dimensional
 * array with a coord in the i-th row ( where a row is parallel to the x-axis)
...
public class OddBoard extends Board {
	...
	public enum Piece {
		WP, BP, EMPTY, INVALID, WP_CLUST, BP_CLUST
	};
	...
	public void move(Move mm) throws IllegalArgumentException {
		...
		data[m.destRow + SIZE][m.destCol + SIZE] = (m.color == Piece.WP) ? Piece.WP
				: Piece.BP;
	...
	public void determineWinner() {
		...
		// If then node x is root, the value will be negative |parent[x]| = size
		// of component of x
		...
		for (int j = -SIZE; j <= SIZE; j++)
			for (int i = -SIZE; i <= SIZE; i++, crt_index++) {
				...
				if (crt_piece == Piece.INVALID || crt_piece == Piece.EMPTY)
					continue;

				int connection_code = 0; // used to know which neighbours are
											// connected
				if (getPieceAt(i - 1, j - 1) == crt_piece)
					connection_code++;
				if (getPieceAt(i, j - 1) == crt_piece)
					connection_code += 2;
				if (getPieceAt(i - 1, j) == crt_piece)
					connection_code += 4;
				...
				case 3: // one connection, have to look for root
					// find root
					int y = crt_index - SIZE_DATA
							- ((connection_code == 1) ? 1 : 0);
					do {
						int tmp = y;
						y = parent[y];
				...
				case 6: // two connections
					...
					// if root not the same, then we have to connect the second
					// piece
					parent[crt_index] += y;
					if (z == crt_index - 1)
						break;
		...
		num_clusters = 0;
		for (int k = 0; k < parent.length; k++) {
			if (parent[k] <= -MIN_CLUSTER_SIZE)
				num_clusters++;
			int y = k;
			while (y >= 0)
				y = parent[y];
			if (y <= -MIN_CLUSTER_SIZE)
				data[k % SIZE_DATA][k / SIZE_DATA] = (data[k % SIZE_DATA][k
						/ SIZE_DATA] == Piece.WP) ? Piece.WP_CLUST
						: Piece.BP_CLUST;
		}
	...
	public String toString() {
		...
		if (winner == NOBODY) {
			b.append("There are " + numEmpty + " empty spaces left. "
					+ getNameForID(turn) + " to play.\n");
		} else {
			b.append("The winner is " + (getNameForID(winner))
					+ "! Number of pieces is " + num_clusters + ".\n");
		}
		for (int j = SIZE; j >= -SIZE; j--) {
			for (int i = 0; i < -j; i++)
				b.append(" ");
